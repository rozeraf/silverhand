...
const TIMELINE_EVENTS = [
  {
    ...
    title: "FORMATION",
    description:
      'Основана Джонни Сильверхендом и Керри Евродином в задних комнатах "Rainbow Cadenza". Их сырая энергия и антикорпоративные тексты мгновенно нашли отклик у бесправной молодежи Найт-Сити.',
  },
  {
    year: "2008",
    title: "THE BREAKUP",
    description:
      "После того как клавишница Нэнси была арестована за то, что выбросила мужа-абьюзера из окна, группа распалась. 7 месяцев тюрьмы для неё означали конец Samurai.",
  },
  {
    year: "2023",
    title: "ARASAKA TOWER",
    description:
      "Джонни возглавил ударную группу Militech, чтобы проникнуть в сердце Арасака-Тауэр и спасти Альт Каннингем. Миссия закончилась ядерным взрывом, уничтожившим центр города, и физической смертью Джонни.",
  },
  {
    year: "2077",
    title: "THE REUNION",
    description:
      'Спустя десятилетия выжившие участники воссоединились для одного последнего концерта в баре Red Dirt. Используя тело Ви как проводник для Джонни, они сыграли "A Like Supreme" в последний раз.',
  },
];
    ...
    if (audioRef.current) {
      if (isPlaying) {
        audioRef.current
          .play()
          .catch((e) =>
            console.warn("Ensure audio files exist in public/music folder", e),
          );
      } else {
        audioRef.current.pause();
      }
      ...
      <div className="flex flex-col lg:flex-row w-full h-full overflow-y-auto lg:overflow-hidden">
        <div className="w-full lg:w-5/12 h-auto lg:h-full relative border-b lg:border-b-0 lg:border-r border-[#222] bg-[#080808] flex flex-col z-10 lg:shadow-[20px_0_50px_rgba(0,0,0,0.9)] shrink-0">
          ...
          <div className="p-4 md:p-6 flex justify-between items-center border-b border-[#222] bg-[#0a0a0a]/80 backdrop-blur-md relative z-20">
            <button
              onClick={onBack}
              className="flex items-center gap-2 text-gray-400 hover:text-[#fcee0a] transition-all group uppercase font-cyber text-[10px] md:text-xs tracking-widest border border-transparent hover:border-[#fcee0a] px-3 py-1 rounded-sm"
            >
              <ArrowLeft
                size={16}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[#ff003c] font-cyber text-[10px] md:text-xs tracking-widest">
                LIVE_FEED
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-8 lg:p-12 relative z-10">
            <div className="relative w-48 md:w-64 lg:w-full max-w-[320px] aspect-square mb-6 md:mb-10 group perspective-1000">
              ...
              ></div>
              <div
                className={`absolute inset-[-5px] md:inset-[-10px] rounded-full border border-[#fcee0a]/20 ${isPlaying ? "animate-spin-reverse-slow" : ""}`}
              ></div>
            ...
            <div className="w-full max-w-[320px] mb-6 md:mb-8 text-center relative px-4">
              ...
              <p className="text-[#ff003c] font-mono text-[10px] md:text-xs tracking-[0.4em] uppercase">
                SAMURAI // SILVERHAND
              </p>
            </div>
          ...
          <div className="max-w-4xl mx-auto p-6 md:p-8 lg:p-16 pt-0 relative z-10">
            <div className="flex flex-col md:flex-row justify-between items-start mb-12 md:mb-16 border-b-2 border-[#333] pb-8 relative">
              <div className="w-full">
                <div className="inline-flex items-center gap-2 mb-2 bg-[#ff003c]/10 px-3 py-1 border border-[#ff003c]/20 rounded-sm">
                  ...
                  <span className="text-[#ff003c] font-mono text-[10px] tracking-widest uppercase">
                    Encrypted Connection
                  </span>
                </div>
                <h1 className="text-4xl sm:text-6xl md:text-8xl font-black font-cyber text-white uppercase leading-[0.85] tracking-tighter mb-4">
                  <span className="block text-stroke-thick text-transparent opacity-30">
                    We Are
                  </span>
                  <span className="relative inline-block">
                    Samurai
                    <span className="absolute -top-3 -right-6 md:-top-4 md:-right-8 text-xl md:text-2xl text-[#fcee0a] font-normal tracking-normal italic opacity-80 rotate-12">
                      est. 2003
                    </span>
                  </span>
                </h1>
                <p className="max-w-md text-gray-400 font-mono text-xs md:text-sm leading-relaxed border-l-2 border-[#fcee0a] pl-4">
                  Хромированный рок. Голос улиц. Мятеж, записанный на пленку.
                  Единственная группа, которая сожгла город, чтобы доказать свою
                  правоту.
                </p>
              </div>

              <div className="mt-6 md:mt-0 opacity-20 hover:opacity-100 transition-opacity duration-500 self-end md:self-auto">
                <img
                  src={ASSETS.samuraiLogo}
                  alt="Logo"
                  ...
                  {TRACKS.map((track, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-4 relative z-10">
                        <span
                          className={`font-mono text-xs w-6 ${currentTrackIdx === idx ? "text-[#fcee0a]" : "text-[#333]"}`}
                        >
                          0{idx + 1}
                        </span>
                        <span className="font-bold font-cyber text-sm md:text-lg tracking-wide uppercase group-hover:text-white transition-colors">
                          {track.title}
                        </span>
                      </div>
                    ...
                    {TIMELINE_EVENTS.map((item, i) => (
                      <div key={i} className="relative group cursor-default">
                        ...
                        <div className="flex flex-col">
                          <span className="text-[#ff003c] font-cyber font-bold text-sm mb-1 group-hover:text-[#fcee0a] transition-colors duration-300">
                            {item.year}
                          </span>
                          <span className="text-gray-300 font-bold text-sm block group-hover:text-white transition-colors duration-300">
                            {item.title}
                          </span>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {MEMBERS.map((member, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between bg-[#111] p-3 border-l-2 border-transparent hover:border-[#00f0ff] transition-colors group"
                      >
                        <div className="flex items-center gap-3">
                          <div className="text-gray-600 group-hover:text-[#00f0ff] transition-colors">
                            {member.icon}
                          </div>
                          <span className="text-gray-300 font-bold text-xs md:text-sm uppercase group-hover:text-white">
                            {member.name}
                          </span>
                        </div>
                        <span className="text-[10px] font-mono text-gray-600 uppercase group-hover:text-[#00f0ff]">
                          {member.role.split(" ")[0]}
                        </span>
                      </div>
                    ))}
            ...
            <div className="mt-16 md:mt-20 border-t border-[#222] pt-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 opacity-70 md:opacity-50 md:hover:opacity-100 transition-opacity pb-8 md:pb-0">
              <div className="font-mono text-[10px] text-gray-500 max-w-xs">
                ...
                330.1. PROCEED AT OWN RISK.
              </div>
              <div className="text-left md:text-right">
                <div className="font-cyber text-xl md:text-2xl font-bold text-[#333]">
                  NIGHT CITY
                </div>
                <div className="font-mono text-xs text-[#ff003c]">
                  ARCHIVES_2077
                </div>
              </div>
            </div>
